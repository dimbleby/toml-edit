...
- **`_slots.py`** — the **physical slot stream**:
  - `Slot` — base; carries `leading: Trivia`, `_prev` / `_next`
    intrusive linked-list pointers, and `owner_aot_entry`.
...
- **`_container.py`** — `Container` (the abstract base, a `dict`
  ...
  `_body_tail`, `_value`, `_header_ref`, `_inline`. Public
  `Mapping` / `MutableMapping` API; mutation is delegated to
  `_layout_ops` / `_inline_ops`.
